...
const getButtonClasses = (variant: string = "default", size: string = "default") => {
  ...
  const variantClasses = {
    ...
    hero: "bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 shadow-lg hover:shadow-xl",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    if (asChild) {
      return <div className={cn(getButtonClasses(variant, size), className)} />;
    }
    return <button className={cn(getButtonClasses(variant, size), className)} ref={ref} {...props} />;
  },
);
